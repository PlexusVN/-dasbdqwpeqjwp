...
<div class="login-overlay" id="loginOverlay">
  <div class="login-box">
    ...
    <button class="btn btn-primary" style="width: 100%; margin-top: 8px; padding: 14px;" onclick="login()">SIGN IN</button>
    <div style="text-align: center; margin-top: 24px;">
      <button class="btn btn-sm" onclick="checkHealth()"><i data-feather="activity"></i> SERVER STATUS</button>
    </div>
  </div>
</div>
...
<header class="topbar">
  <div class="topbar-header">
    <div class="brand"><i data-feather="hexagon" stroke-width="3"></i> PLEXUS SYSTEM</div>
    <div style="display: flex; align-items: center; gap: 24px;">
      <div class="search-bar">
        <i data-feather="search"></i>
        <input type="text" id="globalSearch" placeholder="Search keys, users..." oninput="debounceSearch()" onkeydown="if(event.key==='Enter') debounceSearch()">
      </div>
      <span style="font-weight: 600; font-size: 14px;" id="lastRefresh">AUTO-SYNCING</span>
      <button class="btn btn-sm btn-danger" onclick="logout()"><i data-feather="log-out"></i> EXIT</button>
    </div>
  </div>
  <div class="tabs-container">
    <div class="tab-item active" onclick="nav('dashboard')"><i data-feather="grid"></i> Dashboard</div>
    <div class="tab-item" onclick="nav('keys')"><i data-feather="key"></i> Keys Manager</div>
    <div class="tab-item admin-only" onclick="nav('create')"><i data-feather="plus-square"></i> Create Keys</div>
    <div class="tab-item" onclick="nav('online')"><i data-feather="radio"></i> Online Users</div>
    <div class="tab-item" onclick="nav('products')"><i data-feather="package"></i> Products</div>
    <div class="tab-item" onclick="nav('logs')"><i data-feather="list"></i> Logs</div>
    <div class="tab-item admin-only" onclick="nav('subadmins')"><i data-feather="users"></i> Sub-Admins</div>
  </div>
</header>
...
<main class="main-wrapper">
  ...
  <div id="sec-dashboard" class="section active">
    ...
    <div class="grid-4">
      <div class="card kpi-card">
        <div class="kpi-label">Total Keys <i data-feather="key"></i></div>
        <div class="kpi-value" id="kpiTotal">-</div>
      </div>
      <div class="card kpi-card">
        <div class="kpi-label">Active Keys <i data-feather="check-circle"></i></div>
  ...
  <div id="sec-keys" class="section">
    <div class="section-header">
      ...
      <div class="toolbar-actions">
        <button class="btn btn-success" onclick="exportCSV()"><i data-feather="download"></i> Export CSV</button>
        <button class="btn btn-primary admin-only" onclick="nav('create')"><i data-feather="plus"></i> Generate</button>
      </div>
    </div>
    <div class="toolbar">
      ...
      <div class="toolbar-actions admin-only" id="bulkActions" style="display: none;">
        <span style="font-size: 14px; font-weight: 700; align-self: center;" id="bulkCount">0 selected</span>
        <button class="btn btn-danger" onclick="bulkAction('ban')"><i data-feather="slash"></i> Ban All</button>
        <button class="btn btn-danger" onclick="bulkAction('delete')"><i data-feather="trash"></i> Delete All</button>
      </div>
    </div>
    <div class="table-container">
      <table>
        ...
        <tbody id="keysTableBody">
          <tr><td colspan="9" style="text-align: center; padding: 40px; font-weight: 600;">LOADING DATA...</td></tr>
        </tbody>
      </table>
    </div>
    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 24px; font-weight: 600;">
      <span id="pageInfo">Showing 0 keys</span>
      <div style="display: flex; gap: 12px;">
        <button class="btn btn-sm" onclick="changePage(-1)">PREV PAGE</button>
        <button class="btn btn-sm" onclick="changePage(1)">NEXT PAGE</button>
      </div>
    </div>
  </div>

  <!-- CREATE KEYS -->
  <div id="sec-create" class="section admin-only">
    <div class="section-header">
      <h2 class="section-title">Create New Keys</h2>
    </div>
    <div class="grid-2">
      <div class="card">
        <div style="display: flex; gap: 24px; margin-bottom: 24px; padding-bottom: 16px; border-bottom: 2px solid #000;">
          <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 700;">
            <input type="radio" name="createMode" value="auto" checked onchange="toggleCreateMode()"> GENERATOR
          </label>
          <label style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 700;">
            <input type="radio" name="createMode" value="manual" onchange="toggleCreateMode()"> MANUAL
          </label>
        </div>

        <div id="createAutoForm" onkeydown="if(event.key==='Enter') executeCreateKey()">
          <div class="form-row">
            <div class="form-group" style="flex: 1;"><label>Product</label><select id="cProduct"></select></div>
            <div class="form-group" style="flex: 1;"><label>Key Type</label><select id="cType"><option value="basic">Basic</option><option value="pro">Pro</option><option value="vip">VIP</option></select></div>
          </div>
          <div class="form-row">
            <div class="form-group" style="flex: 1;"><label>Prefix</label><input type="text" id="cPrefix" placeholder="PLX" value="PLX" style="text-transform: uppercase;"></div>
            <div class="form-group" style="flex: 1;"><label>Suffix Blocks</label><select id="cSuffix"><option value="8">2 Blocks</option><option value="12" selected>3 Blocks</option><option value="16">4 Blocks</option><option value="24">6 Blocks</option></select></div>
          </div>
          <div class="form-row">
            <div class="form-group" style="flex: 1;"><label>Duration (Days)</label><input type="number" id="cDays" value="30" min="1"></div>
          ...
          <div class="form-row">
            <div class="form-group" style="flex: 1;"><label>Quantity</label><input type="number" id="cCount" value="1" min="1" max="500"></div>
            <div class="form-group" style="flex: 1;"><label>User Name</label><input type="text" id="cUser" placeholder="Required"></div>
          </div>
          <div class="form-group"><label>Note</label><input type="text" id="cNote" placeholder="Optional context"></div>
        </div>

        <div id="createManualForm" style="display: none;" onkeydown="if(event.key==='Enter') executeCreateKey()">
           <div class="form-row">
            <div class="form-group" style="flex: 1;"><label>Product</label><select id="cmProduct"></select></div>
            <div class="form-group" style="flex: 1;"><label>Key Type</label><select id="cmType"><option value="basic">Basic</option><option value="pro">Pro</option><option value="vip">VIP</option></select></div>
          </div>
          <div class="form-group"><label>Custom Key String</label><input type="text" id="cmKey" placeholder="Enter custom key..." class="mono"></div>
          <div class="form-row">
           ...
           <div class="form-group"><label>User Name</label><input type="text" id="cmUser" placeholder="Required"></div>
        </div>
      ...
      <div class="card" style="display: flex; flex-direction: column;">
        <h3 style="font-size: 18px; font-weight: 700; margin-bottom: 16px; text-transform: uppercase;">Output Console</h3>
  ...
  <div id="sec-online" class="section">
    ...
    <div class="table-container">
      <table>
        <thead>
          <tr><th>KEY</th><th>PRODUCT</th><th>TYPE</th><th>HWID</th><th>USER</th><th>LAST SEEN</th></tr>
        </thead>
  ...
  <div id="sec-products" class="section">
    ...
    <div class="card admin-only" style="margin-bottom: 32px;">
    ...
    <div class="table-container">
      <table>
        <thead><tr><th>ID</th><th>NAME</th><th>SECRET</th><th>CREATED</th><th style="text-align: right;">ACTIONS</th></tr></thead>
  ...
  <div id="sec-logs" class="section">
    ...
    <div class="table-container">
      <table>
        <thead><tr><th>TIMESTAMP</th><th>ACTION</th><th>TARGET KEY</th><th>DETAILS</th></tr></thead>
  ...
  <div id="sec-subadmins" class="section admin-only">
    <div class="section-header">
      <h2 class="section-title">Local Operators</h2>
    </div>
    <div class="card" style="margin-bottom: 32px; max-width: 700px;">
      <div class="form-row" style="margin: 0; align-items: flex-end;" onkeydown="if(event.key==='Enter') addSubAdmin()">
        <div class="form-group" style="margin: 0; flex: 1;"><label>Username</label><input type="text" id="saUser" placeholder="admin2"></div>
        <div class="form-group" style="margin: 0; flex: 1;"><label>Password</label><input type="password" id="saPass" placeholder="••••••••"></div>
        <button class="btn btn-primary" onclick="addSubAdmin()" style="height: 44px; padding: 0 24px;"><i data-feather="user-plus"></i> ADD OPERATOR</button>
      </div>
    </div>
  ...
  <div class="modal">
    ...
    <div class="modal-body" onkeydown="if(event.key==='Enter') actionKey('save')">
      ...
      <div class="grid-2">
        ...
        <div class="form-group"><label>Product</label><div id="emProduct" style="font-weight: 700; font-size: 18px;">-</div></div>
        ...
        <div class="form-group admin-only" style="grid-column: span 2; border-top: 2px solid #000; padding-top: 24px; margin-top: 8px;">
    ...
    <div class="modal-footer">
      <button class="btn admin-only" onclick="actionKey('reset')"><i data-feather="refresh-cw"></i> CLEAR HWID</button>
      <button class="btn btn-danger admin-only" id="emBanBtn" onclick="actionKey('toggleBan')"><i data-feather="slash"></i> BAN</button>
      <button class="btn btn-danger admin-only" onclick="actionKey('delete')"><i data-feather="trash"></i> PURGE</button>
      <button class="btn btn-primary admin-only" onclick="actionKey('save')"><i data-feather="save"></i> SAVE</button>
    </div>
  </div>
</div>
...
<script>
  ...
  async function login() {
    ...
    let subs = JSON.parse(localStorage.getItem('rox_subs') || '[]');
    let sub = subs.find(x => x.u === u && x.p === p);
    
    try {
      // Encode password an toàn cho tiếng Việt/Kí tự đặc biệt
    ...
    try {
      // Test Auth bằng cách gọi API, nếu sai Auth sẽ trả về 401 ngay lập tức.
      const r = await fetch(API + '/health', { headers: { 'Authorization': 'Basic ' + token } });
      
      if (r.status === 401) {
        throw new Error('Sai tài khoản hoặc mật khẩu (Unauthorized)');
      }

      // Nếu HTTP status KHÔNG phải 401 (nghĩa là 200 hoặc 500 do Database chưa Setup)
      // -> Nghĩa là mật khẩu ĐÃ ĐÚNG. Chúng ta vẫn cho Login vào để xem bảng lỗi (nếu có).
      ...
      if (!j.success || j.supabase === 'error') {
         showToast('Cảnh báo: Server chưa kết nối được Database! Vui lòng thiết lập biến môi trường.', 'warning');
      } else {
         showToast('Đăng nhập thành công', 'success');
      }
      
    } catch(e) {
      // Nếu Master Auth sai, thử check Sub-admin
      if (sub) {
        const masterToken = localStorage.getItem('rox_master_token');
        if(masterToken) {
          token = masterToken;
          document.getElementById('loginOverlay').style.display = 'none';
          isSubAdmin = true;
          initApp();
          showToast('Đã cấp quyền truy cập Operator', 'success');
          return;
        } else {
          showToast('Sub-admin cần Master login ít nhất 1 lần để lưu mã', 'warning');
        }
      } else {
  ...
  async function bulkAction(action) {
    if(isSubAdmin) return showToast('No permission', 'error');
  ...
  async function executeCreateKey() {
    ...
    if(mode === 'auto') {
      ...
      body = {
        ...
        user: document.getElementById('cmUser').value.trim()
      };
    }
    ...
    btn.disabled = false; btn.innerHTML = '<i data-feather="zap"></i> EXECUTE BUILD'; feather.replace();
  }
  ...
  function openEditModal(keyString) {
    ...
    let hList = k.hwid && k.hwid.startsWith('[') ? JSON.parse(k.hwid) : [];
    ...
    if(k.status === 'banned') { banBtn.innerHTML = '<i data-feather="check"></i> UNBAN'; banBtn.className = 'btn btn-primary admin-only'; }
    else { banBtn.innerHTML = '<i data-feather="slash"></i> BAN'; banBtn.className = 'btn btn-danger admin-only'; }
  ...
  function renderLogs() {
    ...
    if(!dataStore.logs.length) { tbody.innerHTML = \`<tr><td colspan="4" style="text-align: center; padding: 40px; font-weight: 700;">NO LOGS</td></tr>\`; return; }
  ...
  function renderProducts() {
    ...
    if(!dataStore.products.length) { tbody.innerHTML = \`<tr><td colspan="5" style="text-align: center; padding: 40px; font-weight: 700;">NO PRODUCTS</td></tr>\`; return; }
    tbody.innerHTML = dataStore.products.map(p => \`<tr>
      ...
      <td style="text-align: right;"><button class="btn-icon text-danger admin-only" onclick="deleteProduct(\${p.id})"><i data-feather="trash"></i></button></td>
    </tr>\`).join('');
    feather.replace();
  }
  ...
  function renderSubAdmins() {
    ...
    if(!subs.length) { tbody.innerHTML = \`<tr><td colspan="3" style="text-align: center; padding: 40px; font-weight: 700;">NO OPERATORS</td></tr>\`; return; }
    tbody.innerHTML = subs.map((s, i) => \`<tr>
      <td style="font-weight: 700;">\${s.u}</td>
      <td><span class="badge badge-pending">VIEWER</span></td>
      <td style="text-align: right;"><button class="btn-icon text-danger admin-only" onclick="delSubAdmin(\${i})"><i data-feather="trash"></i></button></td>
    </tr>\`).join('');
    feather.replace();
  }
  function addSubAdmin() {
    ...
    const p = document.getElementById('saPass').value.trim();
    if(!u||!p) return showToast('MISSING FIELDS', 'error');
    let subs = JSON.parse(localStorage.getItem('rox_subs') || '[]');
    subs.push({u, p});
    localStorage.setItem('rox_subs', JSON.stringify(subs));
    document.getElementById('saUser').value=''; document.getElementById('saPass').value='';
    renderSubAdmins(); showToast('OPERATOR ADDED');
  }
  function delSubAdmin(i) {
    if(isSubAdmin) return showToast('No permission', 'error');
    let subs = JSON.parse(localStorage.getItem('rox_subs') || '[]');
    subs.splice(i, 1);
    localStorage.setItem('rox_subs', JSON.stringify(subs));
    renderSubAdmins(); showToast('REMOVED');
  }
