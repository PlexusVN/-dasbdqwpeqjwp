...
<input type="number" id="createMaxDevices" value="1" min="0" style="flex:1">
<label style="font-size:12px;color:#886666;display:flex;align-items:center;gap:4px;cursor:pointer;white-space:nowrap">
<input type="checkbox" id="createUnlimitedDevices" onchange="toggleUnlimited('create')"> Không giới hạn
</label>
</div>
</div>
<div style="display:flex;flex-direction:column;gap:4px">
...
<div style="font-size:11px;color:#886666;margin-top:2px">Tùy chọn: <input type="number" id="createManualDays" value="30" min="1" max="3650" style="width:70px;padding:4px 6px;font-size:11px"> ngày | <input type="number" id="createManualMaxDevices" value="1" min="0" style="width:60px;padding:4px 6px;font-size:11px"> thiết bị <label style="font-size:11px;color:#886666;display:inline-flex;align-items:center;gap:3px;cursor:pointer;margin-left:4px"><input type="checkbox" id="createManualUnlimitedDevices" onchange="toggleUnlimited('manual')"> Unlimited</label></div>
</div>
<div style="display:flex;flex-direction:column;gap:4px">
<label style="color:#aa7777;font-size:11px;font-weight:600;letter-spacing:0.3px;white-space:nowrap;">LOẠI KEY</label>
...
<div class="row"><label>Số TB Tối Đa</label><input type="number" id="editMaxDevices" value="1" min="0" style="max-width:80px"><label style="font-size:12px;color:#886666;display:flex;align-items:center;gap:3px;cursor:pointer"><input type="checkbox" id="editUnlimitedDevices" onchange="toggleUnlimited('edit')"> Unlimited</label><button class="btn-primary btn-sm" onclick="saveMaxDevices()">Lưu</button></div>
...
function checkHealth(){fetch(BASE+'/api/health').then(r=>r.json()).then(d=>{var s=d.supabase?' Supabase: '+d.supabase:'';var k=d.keys_count!=null?' Keys: '+d.keys_count:'';var p=d.products?' Products: '+d.products.join(', '):'';showToast('Server OK |'+s+k+p,'success')}).catch(e=>{showToast('Server không phản hồi','error')})}
...
function refreshData(){const params=new URLSearchParams();const pid=document.getElementById('productFilter').value;if(pid)params.set('product_id',pid);const st=document.getElementById('statusFilter').value;if(st&&st!=='all')params.set('status',st);Promise.all([api('/api/products').then(r=>{allProducts=r.data||[];loadProductFilter();loadCreateProductSelect()}),api('/api/keys?'+params.toString()).then(r=>allKeys=r.data||[]),api('/api/logs').then(r=>allLogs=r.data||[]),api('/api/stats').then(r=>{if(r.data)renderStats(r.data)})]).then(()=>{renderTable();renderLogs();refreshProductTable()}).catch(e=>{console.error(e);showToast('Lỗi tải dữ liệu','error')})}
...
function renderOnline(d){const t=document.getElementById('onlineTableBody');if(!t)return;if(!d.length){t.innerHTML='<tr><td colspan="9" style="text-align:center;color:#886666">Không có key online</td></tr>';return}t.innerHTML=d.map((x,i)=>{var s_=st(x);var stxt='<span style="color:'+(s_.cls==='status-active'?'#33cc33':s_.cls==='status-pending'?'#aa7777':'#dd3333')+'">'+s_.txt+'</span>';const ls=x.last_seen?new Date(x.last_seen).toLocaleTimeString('vi-VN'):'-';const tp=x.type||'basic';var di=getDevInfo(x.hwid);var md=(x.max_devices==null||x.max_devices===undefined)?1:x.max_devices;var mdLabel=md===0?'Không giới hạn':md+' TB';var pn=getProductName(x.product_id);var pc=getProductClass(x.product_id);var hwidDisplay=di.count>0?'<span style="font-family:monospace;font-size:11px;color:#cc6666" title="'+di.list.join(', ')+'">'+di.list.join(', ')+'</span>':'-';return '<tr><td>'+(i+1)+'</td><td class="copy-field">'+x.key+'</td><td><span class="product-badge '+pc+'">'+pn+'</span></td><td style="font-size:11px">'+(tp==='vip'?'<span style="color:#ffcc00;font-weight:600">VIP</span>':tp==='pro'?'<span style="color:#cc66ff;font-weight:600">Pro</span>':'<span style="color:#aa7777">Basic</span>')+'</td><td style="font-size:11px">'+(di.count>0?'<span title="'+di.list.join(', ')+'" style="cursor:help">'+di.count+'/'+mdLabel+'</span>':'<span style="color:#666">0/'+mdLabel+'</span>')+'</td><td style="font-family:monospace;font-size:11px;color:#cc6666">'+hwidDisplay+'</td><td>'+(x.user||'-')+'</td><td>'+ls+'</td><td>'+stxt+'</td></tr>'}).join('')}
...
function saveMaxDevices(){const x=editKeyData;if(!x)return;const unlimited_edit=document.getElementById('editUnlimitedDevices').checked;const md=unlimited_edit?0:parseInt(document.getElementById('editMaxDevices').value);if(isNaN(md)||md<0)return showToast('Số thiết bị không hợp lệ (0 = Unlimited)','error');api('/api/keys/'+encodeURIComponent(x.key)+'/max-devices',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({max_devices:md})}).then(r=>{showToast(r.message,r.success?'success':'error');if(r.success){x.max_devices=md;refreshData();closeEditPanel()}})}
