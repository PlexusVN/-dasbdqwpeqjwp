...
function renderOnline(d){const t=document.getElementById('onlineTableBody');if(!t)return;if(!d.length){t.innerHTML='<tr><td colspan="9" style="text-align:center;color:#886666">Không có key online</td></tr>';return}t.innerHTML=d.map((x,i)=>{var s_=st(x);var stxt='<span style="color:'+(s_.cls==='status-active'?'#33cc33':s_.cls==='status-pending'?'#aa7777':'#dd3333')+'">'+s_.txt+'</span>';const ls=x.last_seen?new Date(x.last_seen).toLocaleTimeString('vi-VN'):'-';const tp=x.type||'basic';var di=getDevInfo(x.hwid);var md=x.max_devices||1;var mdLabel=md===0?'Không giới hạn':md+' TB';var pn=getProductName(x.product_id);var pc=getProductClass(x.product_id);return '<tr><td>'+(i+1)+'</td><td class="copy-field">'+x.key+'</td><td><span class="product-badge '+pc+'">'+pn+'</span></td><td style="font-size:11px">'+(tp==='vip'?'<span style="color:#ffcc00;font-weight:600">VIP</span>':tp==='pro'?'<span style="color:#cc66ff;font-weight:600">Pro</span>':'<span style="color:#aa7777">Basic</span>')+'</td><td style="font-size:11px">'+(di.count>0?'<span title="'+di.list.join(', ')+'" style="cursor:help">'+di.count+'/'+mdLabel+'</span>':'<span style="color:#666">0/'+mdLabel+'</span>')+'</td><td style="font-family:monospace;font-size:11px;color:#cc6666">'+(x.hwid||'-')+'</td><td>'+(x.user||'-')+'</td><td>'+ls+'</td><td>'+stxt+'</td></tr>'}).join('')}
