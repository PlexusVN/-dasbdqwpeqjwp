...
app.get('/api/subadmins', requireAdmin, async (req, res) => {
  ...
  try {
    const { data, error } = await supabase.from('sub_admins').select('username, created_at').order('created_at', { ascending: false });
    if (error) throw error;
...
app.post('/api/subadmins', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const { error } = await supabase.from('sub_admins').insert({ username, password });
    if (error) {
...
app.get('/api/keys', requireAdmin, async (req, res) => {
  try {
    ...
    res.json({ success: true, data: keysWithDevices, total: keysWithDevices.length });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
  }
});
  ...
  <div id="sec-subadmins" class="section admin-only">
    ...
    <div class="card" style="margin-bottom: 32px; max-width: 700px;">
      <div class="form-row" style="margin: 0; align-items: flex-end;" onkeydown="if(event.key==='Enter') addSubAdmin()">
        <div class="form-group" style="margin: 0; flex: 1;"><label>Tên đăng nhập</label><input type="text" id="saUser" placeholder="admin2"></div>
        <div class="form-group" style="margin: 0; flex: 1;"><label>Mật khẩu</label><input type="password" id="saPass" placeholder="••••••••"></div>
        <button class="btn btn-primary" onclick="addSubAdmin()" style="height: 44px; padding: 0 24px;"><i data-feather="user-plus"></i> THÊM TÀI KHOẢN</button>
      </div>
    </div>
    <div class="table-container" style="max-width: 700px;">
      <table>
        <thead><tr><th>TÊN TÀI KHOẢN</th><th>CHỨC VỤ</th><th style="text-align: right;">THAO TÁC</th></tr></thead>
  ...
  async function renderSubAdmins() {
    ...
    try {
      ...
      if(!data.success || !data.data.length) {
        tbody.innerHTML = '<tr><td colspan="3" style="text-align: center; padding: 40px; font-weight: 700;">CHƯA CÓ TÀI KHOẢN CẤP DƯỚI</td></tr>';
        return;
      }
