...
<table><thead><tr><th>STT</th><th>MÃ KEY</th><th>LOẠI</th><th>TRẠNG THÁI</th><th>NGÀY TẠO</th><th>HẾT HẠN</th><th>THIẾT BỊ</th><th>HWID</th><th>NGƯỜI DÙNG</th><th>GHI CHÚ</th><th>HÀNH ĐỘNG</th></tr></thead>
<tbody id="keyTableBody"><tr><td colspan="11" class="loading">Đang tải dữ liệu...</td></tr></tbody></table>
</div>
</div>
</div>
...
function renderTable(){const s=document.getElementById('searchInput').value.toLowerCase(),f=document.getElementById('statusFilter').value;let k=allKeys;if(s)k=k.filter(x=>x.key.toLowerCase().includes(s));if(f!=='all')k=k.filter(function(x){var s_=st(x);return s_.cls==='status-'+f||s_.txt.toLowerCase()===f});const t=document.getElementById('keyTableBody');if(!k.length){t.innerHTML='<tr><td colspan="11" style="text-align:center;color:#886666">Không có dữ liệu</td></tr>';return}t.innerHTML=k.map((x,i)=>{var s_=st(x);var c=s_.cls,stxt=s_.txt,tp=x.type||'basic';var di=getDevInfo(x.hwid);var md=x.max_devices||1;var devDisplay=di.count>0?'<span title="'+di.list.join(', ')+'" style="cursor:help">'+di.count+'/'+md+' TB</span>':'<span style="color:#666">0/'+md+' TB</span>';var hwidDisplay=di.count>0?'<span style="font-family:monospace;font-size:11px;color:#cc6666" title="'+di.list.join(', ')+'">'+di.list.join(', ')+'</span>':'-';return '<tr><td>'+(i+1)+'</td><td class="copy-field">'+x.key+'</td><td style="font-size:11px">'+(tp==='vip'?'<span style="color:#ffcc00;font-weight:600">VIP</span>':tp==='pro'?'<span style="color:#cc66ff;font-weight:600">Pro</span>':'<span style="color:#aa7777">Basic</span>')+'</td><td class="'+c+'">'+stxt+'</td><td>'+new Date(x.created_at).toLocaleDateString('vi-VN')+'</td><td>'+(x.expires_at?new Date(x.expires_at).toLocaleDateString('vi-VN'):'<span style=color:#666>Chưa kích hoạt</span>')+'</td><td style="font-size:11px">'+devDisplay+'</td><td style="font-family:monospace;font-size:11px;color:#cc6666">'+hwidDisplay+'</td><td>'+(x.user||'-')+'</td><td>'+(x.note||'-')+'</td><td nowrap><button class="action-btn" onclick="showEditPanel(\\''+x.key.replace(/'/g,"\\'")+'\\')">Chỉnh sửa</button></td></tr>'}).join('')}
