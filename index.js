...
<script>
  ...
  function renderOnline() {
    ...
    tbody.innerHTML = dataStore.online.map(k => \`<tr>
      <td class="mono copyable" onclick="copy('\${esc(k.key)}')">\${esc(k.key)}</td>
      <td><span class="badge" style="background: #e5e7eb;">\${esc(pMap[k.product_id]||'UNK')}</span></td>
      <td style="font-weight: 700; text-transform: uppercase;">\${esc(k.type||'BASIC')}</td>
      <td class="mono">\${esc(k.hwid)}</td>
      <td style="font-weight: 600;">\${esc(k.user||'-')}</td>
  ...
  function renderProducts() {
    ...
    tbody.innerHTML = dataStore.products.map(p => \`<tr>
      ...
      <td style="font-weight: 700; text-transform: uppercase;">\${esc(p.name)}</td>
      <td class="mono copyable" onclick="copy('\${esc(p.secret)}')">\${esc(p.secret)}</td>
