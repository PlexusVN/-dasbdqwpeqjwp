  ...
  try {
    ...
    if (custom_key) {
      ...
      const { error: insertError } = await supabase.from('keys').insert({
        ...
        user: creator, note: note || '', type: keyType,
      });
      if (insertError) {
      ...
      await supabase.from('activity_log').insert({
        action: 'create', key: trimmedKey, detail: `[Creator: ${creator}] [Type: ${keyType}] [Product: ${product_id}] Created key manually, ${days} days | IP: ${req.ip}`, ip: req.ip,
      });
    ...
    await supabase.from('activity_log').insert({
      ...
      detail: `[Creator: ${creator}] [Type: ${keyType}] [Product: ${product_id}] Created ${num} key(s), ${days} days. Note: ${note} | IP: ${req.ip}`, ip: req.ip,
    });
...
<style>
  ...
  * { 
  ...
  @media (max-width: 768px) {
    ...
    /* Topbar & Header */
    .topbar-header { flex-direction: column; align-items: stretch; gap: 16px; padding: 16px; }
...
<header class="topbar">
  <div class="topbar-header">
    ...
    <div style="display: flex; align-items: center; gap: 24px;">
      ...
      <button class="btn btn-sm btn-danger hide-on-mobile" onclick="logout()"><i data-feather="log-out"></i> EXIT</button>
    </div>
  </div>
  ...
  <div class="tabs-container" id="mobileTabs">
    ...
    <div class="tab-item" onclick="nav('create')"><i data-feather="plus-square"></i> Create Keys</div>
    ...
    <div class="tab-item show-on-mobile" onclick="logout()" style="color: var(--danger); font-weight: 700; border-top: 1px solid #e5e7eb; border-bottom: 2px solid transparent; margin-top: 12px;"><i data-feather="log-out"></i> Exit System</div>
  </div>
</header>
  ...
  <div id="sec-keys" class="section">
    <div class="section-header">
      ...
      <div class="toolbar-actions">
        ...
        <button class="btn btn-primary" onclick="nav('create')"><i data-feather="plus"></i> Generate</button>
      </div>
    </div>
    <div class="toolbar">
  ...
  <div id="sec-create" class="section">
    ...
    <div class="grid-2">
      <div class="card">
        ...
        <div id="createAutoForm" onkeydown="if(event.key==='Enter') executeCreateKey()">
          ...
          <div class="form-group"><label>Quantity</label><input type="number" id="cCount" value="1" min="1" max="500"></div>
    ...
    <div class="card" style="margin-top: 32px;">
      ...
      <div class="table-container">
        <table>
          <thead>
            <tr><th>Time</th><th>Creator</th><th>Product</th><th>Type</th><th>Keys Generated</th></tr>
          </thead>
...
<script>
  ...
  async function login() {
    ...
    if (sub) {
      ...
      if (masterToken) {
        ...
        isSubAdmin = true;
        currentAdminName = sub.u;
        initApp();
        showToast('Đã cấp quyền truy cập Operator', 'success');
        return;
      } else {
        return showToast('Sub-admin cần Master login ít nhất 1 lần để lưu mã', 'warning');
      }
    }
  ...
  function renderCreateHistory() {
    ...
    if (!createLogs.length) {
      tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; padding: 20px; font-weight: 700;">NO HISTORY</td></tr>';
      return;
    }
    const pMap = {}; dataStore.products.forEach(x => pMap[x.id] = x.name);
